...
| IndexNow key の Content-Length 修正 | ✅（本 PR 以降） |
| 他4サイト向けパッチ + `git am` 検証 | ✅ |
| IndexNow（darekore / machi-list / busselect） | ✅ |
| 監視 / apply / completer / sitemap-auto | ✅ master |
| 他リポジトリ push / GSC UI / Site Creator | ❌ 認証・権限不足 |

## 最短完了（Windows・`gh auth login` 済み）
